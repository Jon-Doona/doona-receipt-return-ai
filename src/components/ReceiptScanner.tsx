...
// Updated categories to match your "droplist" and "RAW" sheet logic
...
export const ReceiptScanner = ({ userEmail }: { userEmail: string }) => {
  ...
  const [tripData, setTripData] = useState({
    ...
    destination: '',
    reason: '', // This maps to the "Form" details
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Professional data mapping: checks multiple keys to ensure the fields fill correctly
      if (data) {
        setScanResult({
          amount_ils: (data.amount_ils || data.total_ils || '').toString(),
          original_amount: (data.amount_raw || data.original_amount || '').toString(),
          original_currency: data.currency || data.original_currency || '',
      ...
      toast({ title: "Scan Failed", description: "Manual entry required.", variant: "destructive" });
    } finally {
      setIsScanning(false);
    }
  };
  ...
  const handleFinalSave = async () => {
    setIsSaving(true);
    try {
      // Sends data to the RAW sheet (functions behind the scenes) and the Form
      ...
      setPreview(null);
      setScanResult({
        amount_ils: '', original_amount: '', original_currency: '', description: '',
        date: new Date().toISOString().split('T')[0], category: 'ארוחות'
      });
    ...
    <Card className="p-8 max-w-xl mx-auto space-y-6 text-left">
      <div className="flex justify-between items-start border-b pb-4">
        <div>
          ...
          <p className="text-lg font-semibold">{tripData.startDate} to {tripData.returnDate}</p>
        </div>
      ...
      {!preview ? (
        <div className="py-20 border-2 border-dashed rounded-2xl flex flex-col items-center justify-center bg-slate-50">
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="flex gap-2">
                <Input className="w-16 px-2 text-center font-mono text-xs uppercase" placeholder="CNY" value={scanResult.original_currency} onChange={(e) => setScanResult({...scanResult, original_currency: e.target.value})} />
            ...
            <div>
              ...
              <Input className="border-blue-400 border-2 font-bold bg-blue-50/50" type="number" value={scanResult.amount_ils} onChange={(e) => setScanResult({...scanResult, amount_ils: e.target.value})} />
            </div>
          </div>
          ...
          <div className="flex gap-4 pt-2">
            ...
            <Button className="flex-[2] bg-emerald-600 hover:bg-emerald-700 shadow-md" onClick={handleFinalSave} disabled={isSaving || isScanning}>
              {isSaving ? <Loader2 className="animate-spin mr-2" /> : <Check className="mr-2" />} Save to Spreadsheet
            </Button>
          </div>
        </div>
      )}
